...
def build_BaselineCNN_model(x, keep_prob, y_, output_node_name,num_channels,num_labels,window_size,learning_rate,kernel_size):
    ...
    ##BASE_LINE###
    flatten = tf.reshape(conv4, [-1, 34*num_channels*64])
    fc = tf.layers.dense(flatten, 128, activation=tf.nn.relu)
    dropout = tf.layers.dropout(fc, keep_prob)
    flatten2 = tf.reshape(dropout, [-1, 128])
    fc2 = tf.layers.dense(flatten2, 128, activation=tf.nn.relu)
    dropout2 = tf.layers.dropout(fc2, keep_prob)
...
def main():
    ...
    train_step, loss, accuracy,outputs = build_BaselineCNN_model(x=x, keep_prob=keep_prob,
